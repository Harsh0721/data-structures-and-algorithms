...
int retrieveMin(int node, int nl,int nr, int l,int r  ){
    ...
    int x = retrieveMin(2*node,nl,mid,l,r);
    int y = retrieveMin(2*node+1,mid+1,nr,l,r);

    return min(x,y);

}

void update(int node, int idx, int l,int r, int newVal){
    ...
    if(l<=idx && idx <= mid){
        update(2*node,idx,l,mid,newVal);
        tree[node] = min(tree[node] , tree[2*node] );
    }
    else
    {
        update(2*node+1,idx,mid+1,r,newVal);
        tree[node] = min(tree[node] , tree[2*node+1] );
    }
...
int main()
{
    ...
    FOR1(T,TC)
    {
        ...
        FOR1(i,q){
            ...
            if(query == 'q' ){
                cout<<retrieveMin(1,1,n,a,b)<<endl;
            }
            else
            {
                update(1,a,1,n,b);
