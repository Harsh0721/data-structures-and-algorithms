...
int Solve(int cap_i  , int mask )
{
    if(mask == alldone )return 1; // everyone is assigned a cap

    if(cap_i > 100)return 0;

    if( dp[cap_i][mask] != 0 )return dp[cap_i][mask];

    int ans = Solve(cap_i + 1 , mask ); // don't let anyone wear this cap

    for(int i=0; i<adj[ cap_i].size() ; i++ ) 
    {
        int person = adj[cap_i][i];

        if(  ! (mask & (1<<person)  )    ) // i.e ith person haven't wear any cap till now
        {
            ans = ans + Solve(cap_i + 1 , ( mask | (1<<person) ) ) ;
            if(ans > MOD)ans -= MOD;
        }

    }

    return dp[cap_i][mask] = ans;
