...
int main()
{
    ...
    FOR1(T,TC)
    {
        ...
        vector< vector<ll> > adj(n+1) , dp(n+1),next(n+1);
            ...
            for(int i=0;i<=n;i++)
            {
                for(int j=0;j<=n;j++)
                {
                    ...
                    if( dp[i][k] + dp[k][j] < dp[i][j] )
                    {
                        ...
                        next[i][j] = next[i][k];    // go from i to k and then further ahead
                    }
